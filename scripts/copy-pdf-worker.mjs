// Copy pdfjs-dist's legacy bundled worker into public/ so PdfPreview can
// reference it at /pdf.worker.min.mjs. The legacy build ships polyfills
// (core-js) for older iOS Safari that the modern build assumes are
// available natively. Runs as a postinstall hook so a fresh `npm install`
// always produces the worker; the file itself is gitignored.
